...
def test_orders():
    ...
    Шаги тестирования:
      ...
      2) Для первых двух заказов проверяет, что разница между временем завершения и начала
         не превышает 6 часов.
      3) Для третьего заказа проверяет, что сумма выполненных, возвращенных и ожидающих возврата услуг
         равна общему количеству, а также что выполняется хотя бы одно из условий:
           - выполнено не менее половины услуг;
           - количество возвращенных не превышает выполненных,
             а ожидающих возврата не превышает возвращенных.
    ...
    for i in range(min(2, len(orders["data"]))):
        order = orders["data"][i]
        started_at = parser.isoparse(order["startedAt"])
        completed_at = parser.isoparse(order["completedAt"])
...
def make_report():
    ...
    order_ids.append("326b23a1-e6ab-4b4a-84a1-a3ecb33afc97")  # пример добавления еще одного ID
